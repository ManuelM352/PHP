...
<body style="background-color: lightgreen;">
      <?php
        ...
        if(ISSET($_POST["id"]) && is_numeric($_POST["id"])){
          ...
          if($dao->eliminar($_POST["id"])){
            $_SESSION["msj"]="success-El Plan Semanal ha sido eliminado correctamente";
          }else{
            $_SESSION["msj"]="danger-No se ha podido eliminar el usuario seleccionado";
          }
        }
        $listaPlanSem=$dao->obtenerTodos($userid);
      ?>
      ...
      <div class="container">
        ...
        <!-- <a class="btn btn-success mt-5 mb-3" href="registroPlan.php">Agregar</a> -->
        <form id="formAgregar" action="registroPlan.php" method="post" style="display: inline;">
          <input type="hidden" name="userid" value="<?php echo $_SESSION["usuario"]; ?>">
          <button type="submit" class="btn btn-success mt-5 mb-3">Agregar</button>
        </form>
        <table id="lista" class="table table-striped table-bordered">
            ...
            <tbody>
              <?php
                ...
                foreach ($listaPlanSem as $planSem){
              
                  echo "<tr><td>".$planSem->nombre_materia."</td>".
                          "<td>".$planSem->lunes."</td>".
                           "<td>".$planSem->martes."</td>".
                           "<td>".$planSem->miercoles."</td>".
                           "<td>".$planSem->jueves."</td>".    
                           "<td>".$planSem->viernes."</td>".   
                           "<td>".$planSem->fechaInicio."</td>".  
                           "<td>".$planSem->fechaFin."</td>".                       
                           "<td><form method='post'>".
                               "<button formaction='registroPlan.php' class='btn btn-primary' name='id' value='".$planSem->id."'>Editar</button>".                               
                               "<button type='button' class='btn btn-danger' onclick='confirmar(this)' name='id' value='".$planSem->id."'>Eliminar</button>".
                               "</form></td></tr>";
              }
                 ?>
